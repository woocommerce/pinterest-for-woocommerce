...
class SavedDataTest extends WP_UnitTestCase {
	...
	/**
	 * API token cached by the calling suite.
	 *
	 * @var mixed
	 */
	...
	public function set_up() {
		...
		Logger::$logger        = null;
		$token                 = new \ReflectionProperty( Base::class, 'token' );
		$token->setAccessible( true );
		$this->previous_token = $token->getValue();
		$token->setValue( null, null );
	...
	public function tear_down() {
		...
		$token          = new \ReflectionProperty( Base::class, 'token' );
		$token->setAccessible( true );
		$token->setValue( null, $this->previous_token );
		$state = new \ReflectionProperty( ProductFeedStatus::class, 'state' );
		$state->setAccessible( true );
	...
	public function test_real_139_upgrade_preserves_historical_state_on_reentry() {
	...
	/** A real WooCommerce editor save submits and persists the attribute form. */
	public function test_product_editor_save_preserves_feed_attributes() {
