...
<p class="form-field <?php echo esc_attr( $input['id'] ); ?>_field pinterest-input-google-category <?php echo esc_attr( $input['wrapper_class'] ); ?>">
	...
	<select class="wc-product-search" style="width: 50%;" id="<?php echo esc_attr( $input['id'] ); ?>" name="<?php echo esc_attr( $input['name'] ); ?>" data-placeholder="<?php esc_attr_e( 'Search for a category&hellip;', 'pinterest-for-woocommerce' ); ?>" data-action="woocommerce_json_search_google_category">
