...
class FeedRegistration {
	 ...
	 * Deregister them if this plugin generated the feed files they point to.
	 *
	 * Data sources the merchant configured manually are left untouched, including the ones
	 * hosted on the store's own domain.
	 *
	 * @param string $feed_id Feed ID to preserve. Pass an empty string to delete every feed
	...
	public static function maybe_delete_stale_feeds_for_merchant( string $feed_id ) {
		...
		foreach ( $feeds as $feed ) {
			...
			/*
			 * Delete the feed only if this plugin generated the feed file it points to.
			 * Merchants may host their own data sources on the store domain, so the site
			 * URL alone does not establish feed ownership.
			 */
			$is_woocommerce_feed = Feeds::is_plugin_generated_feed_location( $feed['location'] ?? '' );
			if ( $is_woocommerce_feed ) {
				Feeds::delete_feed( $registered_feed_id );
